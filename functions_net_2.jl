...
## Network (edge_list)
# function update_ysm_series_net!(w::Array{Float64,1}, p::Float64, N::Int64, edge_list::Vector{Vector{Int64}}, nodes::Vector{Int64}, time::Vector{Int64}, save_time::Vector{Int64})
    ...
    @inbounds for t = time
        @inbounds for tt = t0:t
            ...
            @inbounds for i = 1:N_gc
                # idx[1] = edge_list[ind_list[i]]
                # idx[1] = edge_list[ind_list[i],1]
                # idx[2] = edge_list[ind_list[i],2]
                # idx[1] = idx_list[i]
                # update_ysm!(w, p, idx[1][1], idx[1][2])
                # update_ysm!(w, p, idx[1], idx[2])
